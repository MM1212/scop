...
Swapchain::~Swapchain() {
  ...
  for (uint32_t i = 0; i < depthImages.size(); i++) {
    vkDestroyImageView(device.getHandle(), depthImageViews[i], nullptr);
    vkDestroyImage(device.getHandle(), depthImages[i], nullptr);
    vkFreeMemory(device.getHandle(), depthImageMemorys[i], nullptr);
  }
...
VkResult Swapchain::submitCommandBuffers(
  ...
  if (imagesInFlight[*imageIndex] != VK_NULL_HANDLE) {
    vkWaitForFences(device.getHandle(), 1, &imagesInFlight[*imageIndex], VK_TRUE, UINT64_MAX);
  }
...
void Swapchain::createImageViews() {
  ...
  for (size_t i = 0; i < swapChainImages.size(); i++) {
    ...
    if (vkCreateImageView(device.getHandle(), &viewInfo, nullptr, &swapChainImageViews[i]) !=
      VK_SUCCESS) {
...
void Swapchain::createDepthResources() {
  ...
  for (uint32_t i = 0; i < depthImages.size(); i++) {
    ...
    if (vkCreateImageView(device.getHandle(), &viewInfo, nullptr, &depthImageViews[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture image view!");
    }
  }
}
...
void Swapchain::createSyncObjects() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateSemaphore(device.getHandle(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) !=
      VK_SUCCESS ||
